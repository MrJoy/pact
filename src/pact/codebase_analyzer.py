    ...
    for m in _TS_CONST_RE.finditer(source):
        ...
        if kind == "arrow":
            ...
            if paren_idx >= 0:
                ...
                #
                # Only when the token after that balanced list is `:` or `=>`,
                # though. An IIFE-assigned const --
                # `const x = (() => { ... })()` -- puts its own parens where a
                # parameter list would be, but follows them with another `(`.
                # In that case the first arrow after the assignment is the IIFE's.
                # For a real parameter list, delimiter-aware search skips arrows
                # nested in a function-valued return type as well.
                params_close = _match_bracket(masked, abs_paren)
                if params_close > 0:
                    following = masked[params_close + 1:].lstrip()
                    if following.startswith((":", "=>")):
                        outer_arrow = _find_ts_outer_arrow(
